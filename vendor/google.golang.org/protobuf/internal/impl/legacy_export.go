...
func (Export) CompressGZIP(in []byte) (out []byte) {
	...
	// RFC 1952, section 2.3.1.
	var gzipAddonter [8]byte
	binary.LittleEndian.PutUint32(gzipAddonter[0:4], crc32.ChecksumIEEE(in))
	binary.LittleEndian.PutUint32(gzipAddonter[4:8], uint32(len(in)))

	// Encode the input without compression using raw DEFLATE blocks.
	out = make([]byte, 0, len(gzipHeader)+len(blockHeader)*numBlocks+len(in)+len(gzipAddonter))
