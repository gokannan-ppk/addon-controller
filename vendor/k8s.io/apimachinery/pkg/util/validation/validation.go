...
// (e.g. acme.io/addon). All characters before the first "/" must be a valid
...
func IsDomainPrefixedPath(fldPath *field.Path, dpPath string) field.ErrorList {
	...
	if len(segments) != 2 || len(segments[0]) == 0 || len(segments[1]) == 0 {
		return append(allErrs, field.Invalid(fldPath, dpPath, "must be a domain-prefixed path (such as \"acme.io/addon\")"))
	}
...
// examples:
// - valid: *.bar.com, *.addon.bar.com
// - invalid: *.*.bar.com, *.addon.*.com, *bar.com, f*.bar.com, *
