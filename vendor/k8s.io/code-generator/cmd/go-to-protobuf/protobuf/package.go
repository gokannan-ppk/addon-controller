...
func newProtobufPackage(packagePath, packageName string, generateAll bool, omitFieldTypes map[types.Name]struct{}) *protobufPackage {
	pkg := &protobufPackage{
		DefaultPackage: generator.DefaultPackage{
			// The protobuf package name (addon.bar.baz)
			PackageName: packageName,
			// A path segment relative to the GOPATH root (addon/bar/baz)
...
func (p *protobufPackage) ProtoTypeName() types.Name {
	return types.Name{
		Name:    p.Path(),       // the go path "addon/bar/baz"
		Package: p.Name(),       // the protobuf package "addon.bar.baz"
