...
//
// Or if the type, in package addon, is "type Bar struct { ... }", then the raw
// namer will return "addon.Bar" as the name of the type, and if 'tracker' was
// not nil, will record that package addon needs to be imported.
...
type NameStrategy struct {
	...
	// ignored.
	//
	// For example, if Ignore words lists "proto" and type Addon is in
	// pkg/server/frobbing/proto, then a value of 1 will give a type name
	// of FrobbingAddon, 2 gives ServerFrobbingAddon, etc.
