...
const (
	...
	//   type Addon string
	//   type Bar Addon
	// Bar is an alias of Addon.
	//
	// In the real go type system, Addon is a "Named" string; but to simplify
	// generation, this type system will just say that Addon *is* a builtin.
	// We then need "Alias" as a way for us to say that Bar *is* a Addon.
	Alias Kind = "Alias"
